...
<body>
    ...
    <main>
        <div class="blog">
            <h1 class="text-center m-3">Your recently posts:</h1>
            <?php
            $posts = load_post_by_user_id($_SESSION["user_id"]);
            $len = count($posts);
            $n_items = 5;
            if (!$posts) {
                ...
                <ul class="pagination justify-content-center">
                    <li class="page-item">
                        <a class="page-link" href="./user.php?page=' . ($_GET["page"] - 1 >= 1 ? $_GET["page"] - 1 : 1) . '" aria-label="Previous">
                            <span aria-hidden="true">&laquo;</span>
                            <span class="sr-only">Previous</span>
                        </a>
                    </li>';
                for ($i = 1; $i <= intdiv($len, $n_items) + 1; $i += 1) {
                    $pagination = $pagination . '<li class="page-item"><a class="page-link" href="./user.php?page=' . $i . '">' . $i . '</a></li>';
                }

                $pagination = $pagination . '<li class="page-item">
                        <a class="page-link" href="./user.php?page=' . ($_GET["page"] + 1 <= intdiv($len, $n_items) + 1 ? $_GET["page"] + 1 : intdiv($len, $n_items) + 1) . '" aria-label="Next">

...
<body>
    ...
    <main>
        ...
        <div class="blog">
            <?php
            $render = '<div class="category">
            <div class="category-content">';
            if (!$_GET["category"]) {
                $categories = get_category();
                foreach ($categories as $category) {
                    # code...
                    $render = $render . '<h3 class="text-center"><a href="./category.php?category=' . $category["category"] . '">' . $category["category"] . "</a></h3>";
                }
                $render = $render . "</div></div>";
                echo $render;
            } else {
                $posts = load_post_by_category($_GET["category"]);
                $len = count($posts);
                $n_items = 5;
                if (!$posts) {
                ...
                <ul class="pagination justify-content-center">
                    <li class="page-item">
                        <a class="page-link" href="./category.php?category=' . $_GET["category"] . '&page=' . ($_GET["page"] - 1 >= 1 ? $_GET["page"] - 1 : 1) . '" aria-label="Previous">
                            <span aria-hidden="true">&laquo;</span>
                            <span class="sr-only">Previous</span>
                        </a>
                    </li>';
                for ($i = 1; $i <= intdiv($len, $n_items) + 1; $i += 1) {
                    $pagination = $pagination . '<li class="page-item"><a class="page-link" href="./category.php?category=' . $_GET["category"] . '&page=' . $i . '">' . $i . '</a></li>';
                }

                $pagination = $pagination . '<li class="page-item">
                        <a class="page-link" href="./category.php?category=' . $_GET["category"] . '&page=' . ($_GET["page"] + 1 <= intdiv($len, $n_items) + 1 ? $_GET["page"] + 1 : intdiv($len, $n_items) + 1) . '" aria-label="Next">
